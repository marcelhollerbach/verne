...
_emous_type_debug_emous_type_object_get(Eo *obj EINA_UNUSED, void *pd EINA_UNUSED)
{
    ...
    sd = efl_add(EMOUS_TYPE_DEBUG_CLASS, NULL);
...
emous_debug_device_start(void)
{
   ...
   // add this one
   Emous_Device_Debug *db = efl_add(EMOUS_DEVICE_DEBUG_CLASS, NULL);
   devices = eina_list_append(devices, db);
   efl_event_callback_call(sd, EMOUS_TYPE_EVENT_DEVICE_ADDED, db);
   // Add a second one
   db = efl_add(EMOUS_DEVICE_DEBUG_CLASS, NULL);
   devices = eina_list_append(devices, db);
   efl_event_callback_call(sd, EMOUS_TYPE_EVENT_DEVICE_ADDED, db);
   // remove the first one
   efl_event_callback_call(sd, EMOUS_TYPE_EVENT_DEVICE_DELETED, db);
