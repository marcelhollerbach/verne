...
sort_name_func(const void *data1, const void *data2)
{
   ...
   if (config->sort.type == ELM_FILE_SELECTOR_SORT_TYPE_NAME)
     {
        return _file_name_sort(f1, f2);
     }
   else if (config->sort.type == ELM_FILE_SELECTOR_SORT_TYPE_SIZE)
     {
   ...
   else if (config->sort.type == ELM_FILE_SELECTOR_SORT_TYPE_DATE)
     {
        Efm_File_Stat *st1, *st2;
   ...
   if (eo_do_ret(f1, is, efm_file_is_type(EFM_FILE_TYPE_DIRECTORY)) &&
            ...
            !eo_do_ret(f2, is, efm_file_is_type(EFM_FILE_TYPE_DIRECTORY)))
     {
        if (config->sort.folder_placement == ELM_FILE_SELECTOR_FOLDER_PLACEMENT_FIRST)
          return -1;
        else if (config->sort.folder_placement == ELM_FILE_SELECTOR_FOLDER_PLACEMENT_LAST)
          return 1;
        else
          return sort_name_func(f1, f2) * mul;
     }
