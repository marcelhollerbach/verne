         ...
         switch(operation->type)
         {
            ...
            case OP_MOVE:
              ...
              if (operation->type == OP_MOVE)
                efl_file_set(layout, THEME_PATH"/efm.edc.edj", "kvasir.fs_op.move");
              else
                efl_file_set(layout, THEME_PATH"/efm.edc.edj", "kvasir.fs_op.copy");

              // add progressbar
              progress = efl_add(ELM_PROGRESSBAR_CLASS, obj);
              efl_wref_add(progress, &operation->ui.progress);
              // set part and text
              elm_object_part_content_set(layout, "kvasir.progress", progress);
              elm_object_part_text_set(layout, "kvasir.to", operation->goal);
              elm_object_part_text_set(layout, "kvasir.from", operation->from);
              // set progress to correct values
              elm_progressbar_value_set(progress, operation->ui.progress_value);
            break;
            case OP_REMOVE:
              efl_file_set(layout, THEME_PATH"/efm.edc.edj", "kvasir.fs_op.remove");
              elm_object_part_text_set(layout, "kvasir.from", operation->from);
            break;
         }
...
{
    elm_layout_signal_callback_add(layout, "kvasir.fsaction.popup", "theme", _popup_cb, NULL);
